...
class SettingsRepository(private val context: Context) {
    ...
    companion object {
        ...
        val TEXT_SIZE_KEY = stringPreferencesKey("text_size") // small, medium, large, extra_large
        val FEEDSPOT_TOKEN_KEY = stringPreferencesKey("feedspot_token")
    }

    val textSize: Flow<Float> = context.dataStore.data.map { preferences ->
        val str = preferences[TEXT_SIZE_KEY] ?: "1.0"
        str.toFloatOrNull() ?: 1.0f
    }
    ...
    val feedspotToken: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[FEEDSPOT_TOKEN_KEY] ?: ""
    }
    ...
    suspend fun setTextSize(size: Float) {
        context.dataStore.edit { preferences ->
            preferences[TEXT_SIZE_KEY] = size.toString()
        }
    }

    suspend fun setFeedspotToken(token: String) {
        context.dataStore.edit { preferences ->
            preferences[FEEDSPOT_TOKEN_KEY] = token
        }
    }
}
